...
let weather = {
    ...
    displayWeather: function (data) {
        ...
        const { humidity, temp_min, temp_max, pressure } = data.main;
        const { speed } = data.wind;
        const { sunrise, sunset, country } = data.sys;
        console.log(name, icon, description, humidity, speed, temp_min, temp_max, sunrise, sunset, country, pressure);
...
day.fetchDay("Lagos, NG")


// http://openweathermap.org/img/wn/04d@2x.png
// weather.fetchWeather(`${window.navigator.geolocation.getCurrentPosition}`)
// hour.fetchTemp(`${window.navigator.geolocation.getCurrentPosition}`)
// day.fetchDay(`${window.navigator.geolocation.getCurrentPosition}`)
// icon.fetchIcon(`${window.navigator.geolocation.getCurrentPosition}`)
